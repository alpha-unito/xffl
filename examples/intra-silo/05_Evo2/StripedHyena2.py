...
class StripedHyena(nn.Module):
    def __init__(self, config, current_device):
        ...
        for layer_idx in tqdm(range(config.num_layers)):
            ...
            # self.block_idx_to_device[layer_idx] = current_device
                    ...
                    try:
                        ...
                        if isinstance(current_module, TELinear):
                            ...
                            if hasattr(current_module, "fp8_meta"):
                                if (
                                    hasattr(current_module.fp8_meta, "scaling_fwd")
                                    and "scaling_fwd" not in te_fp8_meta
                                ):
                                    te_fp8_meta["scaling_fwd"] = (
                                        current_module.fp8_meta["scaling_fwd"]
                                    )
                                if (
                                    hasattr(current_module.fp8_meta, "scaling_bwd")
                                    and "scaling_bwd" not in te_fp8_meta
                                ):
                                    te_fp8_meta["scaling_bwd"] = (
                                        current_module.fp8_meta["scaling_bwd"]
                                    )

                            if (
                                isinstance(extra_state_value, io.BytesIO)
                                or "recipe" in te_fp8_meta
                                or (
                                    "scaling_fwd" in te_fp8_meta
                                    and "scaling_fwd" not in te_fp8_meta
                                )
                                or (
                                    "scaling_bwd" in te_fp8_meta
                                    and "scaling_bwd" not in te_fp8_meta
                                )
                            ):
                                buffer = io.BytesIO()
                        ...
                        self.logger.debug(
                            f"Could not find module for {k}, skipping recipe injection."
                        )
                    except Exception as e:
                        self.logger.warning(
                            f"Error processing _extra_state for {k}: {e}"
                        )
            ...
            for k in missing_in_state_dict:
                if k.endswith("._extra_state"):
                    ...
                    try:
                        ...
                        self.logger.debug(
                            f"Module for missing key {k} not found. Skipping."
                        )
                    except Exception as e:
                        self.logger.warning(
